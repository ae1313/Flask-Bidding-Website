...
def bidding():
    if request.method == "POST":
        ...
        if bids == [] and bid > min:
            ...
            db.Groceries.update_one({"pageID":id}, {"$push":{"bidding":{"name":username,"bid":bid}}})
            db.users.update_one({"email":session["email"]},{"$push":{"bids":{"amount":bid,"id":data["id"],"car":db.Groceries.find_one({"pageID":data["id"]})["name"]}}})
            return "Good"
        elif bids == []:
            return "False"
        elif bids[0]["bid"] < bid:
            db.Groceries.update_one({"pageID":data["id"]}, {"$push":{"bidding":{"$each":[{"name":username,"bid":bid}],"$position":0}}})
            db.users.update_one({"email":session["email"]},{"$push":{"bids":{"amount":bid,"id":data["id"],"car":db.Groceries.find_one({"pageID":data["id"]})["name"]}}})
